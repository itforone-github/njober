...
import co.kr.itforone.njober.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    boolean instaChk = false;
    String instaLink = "";
    String instaCategory = "";
    String instaIdx = "";

    final String PUBKEY = "911277";//핀크럭스 매체코드
    ...
    public void webViewSetting() {
        ...
        setting.setUserAgentString("Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Mobile Safari/537.36"+"/ANJOBER");
        ...
        chrome = new WebChromeClient() {
            ...
            public boolean onShowFileChooser(
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {// API 24 이상 일경우..
                    File imageStorageDir = new File(MainActivity.this.getFilesDir() + "/njober", "njober");
                    ...
                    File imageStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "njober");
                    if (!imageStorageDir.exists()) {
                        // Create AndroidExampleFolder at sdcard
                        imageStorageDir.mkdirs();
                    }
...
//                    filePathCallbackLollipop.onReceiveValue(null);
    ...
    protected void onResume() {
        ...
        // 일감 완료 후 (instawork.php(변경 전 url) or click.php(변경 후 url))로 돌아갔을 때 아래 함수 실행
        if ((binding.webView.getUrl().contains("instawork.php") || binding.webView.getUrl().contains("click.php")) && instaChk) {
            binding.webView.loadUrl("javascript:ajax_insta_action('"+ instaLink +"', '"+ instaCategory + "', '" + instaIdx + "')");
            instaChk = false;
        }
    ...
    public void onBackPressed() {
        ...
        WebBackForwardList historyList = binding.webView.copyBackForwardList(); // history list
        /*String backTargetUrl = historyList.getItemAtIndex(historyList.getCurrentIndex() - 1).getUrl();
        Log.d("backurl", backTargetUrl);*/
    ...
    class WebJavascriptEvent{
        ...
        @JavascriptInterface
        public void instaOpen(String link, String category, String idx) { // 인스타그램 URL 오픈 -- 인스타그램 일감 페이지에서 일감 클릭 시 해당 URL 이동
            Log.d("instaUrl", link);
            if(link != "") {
                instaChk = true;
                instaLink = link;
                instaCategory = category;
                instaIdx = idx;
                ...
                if(instaLink != "") { // 앱으로 연결
                    ...
                    } catch (ActivityNotFoundException e) { // 앱 없으면 플레이스토어로 연결
        ...
        @JavascriptInterface
        public void goAdView(String mb_id){
            PincruxOfferwall offerwall = PincruxOfferwall.getInstance();
            offerwall.init(MainActivity.this,PUBKEY,mb_id);
...
//        initBannerView(staticBav, "Z0740dkeh9",320,50);
//    }
